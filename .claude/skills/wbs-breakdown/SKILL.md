...
2. **Inventory** — ONE agent parses ALL mockups → authoritative list of every screen, its sub-screens (UI regions), roles, and a stable ID. This is the backbone that prevents missed features (the "M3 was empty" failure). For each screen it also records the **navigable handle** in the mockup's own addressing scheme — a real route/anchor if one exists, else the app-shell `{ screen: "<module>/<tab>", persona? }` descriptor — so every `mockup_ref` *resolves* (can be opened and screenshotted), never a decorative `#fragment`.
